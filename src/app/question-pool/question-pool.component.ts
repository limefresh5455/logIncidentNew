...
export class QuestionPoolComponent implements OnInit {
  ...
  onSort({column, direction}: SortEvent) {
    ...
    if (direction === '' || column === '') {
      this.questionPool = this.questionPool;
    } else {
      this.questionPool = [...this.questionPool].sort((a, b) => {
    ...
    dialogRef.afterClosed().subscribe(response => {
      if (response) {
        ...
        this.mmtService.editQuestionPool(questionId, data).subscribe(res => {
          swal.fire({
            title: 'Question Pool MMT Updated Status',
            icon: 'success'
          });
          this.mmtService.getMmtQuestionPool(this.incidentId).subscribe(resp => {
            this.questionPool = resp.data;
  ...
  openAddQuestionModal(content, id, audittype) {
    ...
    if(id == null){
      ...
      this.mmtQuestionLists = this.questionPool.filter((o2) => id === o2.id);
      console.log('====================================');
      console.log( this.mmtQuestionLists[0].question,);
      console.log('====================================');
       this.createForm.setValue({
  ...
  onSubmit() {
    ...
    if(this.questionId != ""){
      console.log(this.questionId, this.createForm.value)
      this.mmtService.editQuestionPool(this.questionId, this.createForm.value).subscribe(res => {
        swal.fire({
          title: 'Question Pool MMT Updated',
          icon: 'success'
        });
        this.modalService.dismissAll();
        this.router.navigateByUrl('/mmt/question-pool-mmt');
        this.mmtService.getMmtQuestionPool(this.incidentId).subscribe(resp => {
          this.questionPool = resp.data;
      ...
      this.mmtService.createQuestionPool(this.incidentId, this.createForm.value).subscribe(resp => {
        swal.fire({
          title: 'Question Pool MMT Created',
          icon: 'success',
